...
from dataset_twoinputs import DatasetFolder_two
...
def main():
    ...
    pretrained_cfg['file'] = r'shared_models/resnet50_a1_0-14fe96d1.pth'
    model = timm.models.create_model('resnet50', pretrained=True, pretrained_cfg=pretrained_cfg)
    model = model.cuda()
    model = model.eval()
    config = resolve_data_config({}, model=model)
    transform = create_transform(**config)
    
    prem = predict_model(model.fc)
    # feat_exp = feature_model()
    
    ### ID dataloader
    val_image_dir = "./datasets/images/val"
    val_set = DatasetFolder_two(val_image_dir, transform = transform)
    ...
    ood_set = DatasetFolder_two(ood_image_dir, transform = transform)
