...
def preprocess_image(
    img: np.ndarray, mean=[
        0.4850, 0.4560, 0.4060], std=[
            0.2290, 0.2240, 0.2250]) -> torch.Tensor:
    preprocessing = Compose([
        ToTensor(),
        Normalize(mean=mean, std=std)
    ])
...
# def deprocess_image(img):
#     """ see https://github.com/jacobgil/keras-grad-cam/blob/master/grad-cam.py#L65 """
#     channel_mean = [0.4850, 0.4560, 0.4060]
#     channel_std = [0.2290, 0.2240, 0.2250]
#     img[0] = img[0] * channel_std[0] + channel_mean[0]#三个通道分别进行反归一化...按公式来的
#     img[1] = img[1] * channel_std[1] + channel_mean[1]
#     img[2] = img[2] * channel_std[2] + channel_mean[2]
